...
Serialize.isMultiTargetVector = (opts) => {
  return (
    (opts === null || opts === void 0 ? void 0 : opts.targetVector) !== undefined &&
...
Serialize.hybridVector = (args) => {
  ...
  if (Serialize.isHybridVectorSearch(vector)) {
    const { targets, targetVectors, vectorBytes } = Serialize.vectors(
      Object.assign(Object.assign({}, args), { vector: vector })
    );
    ...
    const { targetVectors, targets } = Serialize.vectors(
      Object.assign(Object.assign({}, args), { vector: undefined })
    );
    return {
      targets,
      targetVectors,
    ...
    const { targetVectors, targets, vectorBytes, vectorPerTarget } = Serialize.vectors(
      Object.assign(Object.assign({}, args), { vector: vector.vector })
    );
    return {
      targetVectors,
      targets,
...
Serialize.vectors = (args) => {
  const invalidNearVectorError = new errors_js_1.WeaviateInvalidInputError(`near vector argument can be:
            - an array of numbers
  ...
  if (utils_js_1.NearVectorInputGuards.isObject(args.vector)) {
    ...
    if (utils_js_1.NearVectorInputGuards.is2DArray(args.vector)) {
